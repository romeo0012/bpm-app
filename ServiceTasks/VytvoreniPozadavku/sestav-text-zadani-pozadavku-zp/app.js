    ...
    try {
      ...
      for (const task of tasks) {
        ...
        console.log('Variables:', {
          sluzba: getVar(task, 'sluzba'),
          category: getVar(task, 'category'),
          macdOperace: getVar(task, 'macdOperace')
        });
        var updatedDescription = getVar(task, 'description')
            .replace("TEXT_PROSTREDI", getVar(task, 'rfcEnvironmentValue'))
        ;
        var updatedInformation = getVar(task, 'information')
            .replace("TEXT_SLUZBA", getVar(task, 'sluzba'))
            .replace("TEXT_ZADOSTI", getVar(task, 'rfcZadost'))
            .replace("TEXT_ZALOZKA", getVar(task, 'lldZalozka'))
            .replace("TEXT_RADKY", getVar(task, 'lldRadky'))
            .replace("TEXT_PROJEKT", getVar(task, 'macdOperace'))
            .replace("TEXT_LLD", getVar(task, 'rfcOdkazNaLldCmdb'))
            .replaceAll("TEXT_ZADAVATEL", getVar(task, 'rfcZadavatelZmeny'))
            .replace("TEXT_KOORDINATOR", getVar(task, 'rfcKoordinatorZmeny'))
            .replace("TEXT_RESITEL", getVar(task, 'rfcResitelZmeny'))
            .replace("TEXT_SCHVALOVATEL", getVar(task, 'rfcSchvalovatelZmeny'))
            .replace("TEXT_TERMIN", String(getVar(task, 'rfcTerminNasazeni') ?? "TEXT_TERMIN"))
        ;
