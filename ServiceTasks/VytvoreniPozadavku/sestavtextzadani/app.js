    ...
    try {
      ...
      for (const task of tasks) {
        ...
        console.log('Variables:', {
          ...
          macdOperace: getVar(task, 'macdOperace')
        });

        const rfcEnvironment = task.variables.rfcEnvironment;
        const description = task.variables.description;
        const information = task.variables.information;
        const lldZalozka = task.variables.lldZalozka;
        const lldRadky = task.variables.lldRadky;

        const rfcEnvironmentValue = rfcEnvironment?.value ?? rfcEnvironment;
        const descriptionValue = description?.value ?? description;
        const informationValue = information?.value ?? information;
        const lldZalozkaValue = lldZalozka?.value ?? lldZalozka;
        const lldRadkyValue = lldRadky?.value ?? lldRadky;

        var updatedDescription = String(descriptionValue).replace("TEXT_PROSTREDI", String(rfcEnvironmentValue ?? ""));
        var updatedInformation = "Popis:\n" + updatedDescription + "\n\nDetailní popis:\n" + String(informationValue);
        updatedInformation = String(updatedInformation).replace("TEXT_ZALOZKA", String(lldZalozkaValue ?? ""));
        updatedInformation = String(updatedInformation).replace("TEXT_RADKY", String(lldRadkyValue ?? ""));
        updatedInformation = updatedInformation + "\n\nOdkaz na LLD dokumentaci:\n\n" + task.variables.rfcOdkazNaLldCmdb.value;
        updatedInformation = updatedInformation + "\n\nS pozdravem,\n\nRoman Marciňa"
        updatedInformation = updatedInformation + "\n\nZadavatel změny: " + task.variables.rfcZadavatelZmeny.value;
        updatedInformation = updatedInformation + "\nKoordinátor změny: " + task.variables.rfcKoordinatorZmeny.value;
        updatedInformation = updatedInformation + "\nŘešitel změny: " + task.variables.rfcResitelZmeny.value;
        updatedInformation = updatedInformation + "\nSchvalovatel změny: " + task.variables.rfcSchvalovatelZmeny.value;
        updatedInformation = updatedInformation + "\nTermín nasazení změny: " + task.variables.rfcTermin.value;
        updatedInformation = updatedInformation + "\nPopis nasazení: Tento Změnový požadavek (RfC) bude realizován v režimu Fast Track Change bez nutnosti zřízení CAB."

        const processVariables = {};

        for (const key of Object.keys(task.variables)) {
            if (key === 'description') {
                processVariables[key] = toCamundaValue(updatedDescription);
                console.log(`Updated variable 'description' to: ${updatedDescription}`);
            }
            if (key === 'information') {
                processVariables[key] = toCamundaValue(updatedInformation);
                console.log(`Updated variable 'information' to: ${updatedInformation}`);
            }
        }
